...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "gui" because\n';
 ...
 * "/gui.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['gui'].superdevmode = true;

  var expectedSuffix = '/gui.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + gui + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:gui';
  var devModeSessionKey = '__gwtDevModeSession:gui';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/gui/gui.nocache.js';
  ...
  $wnd.__gwt_activeModules['gui'].canRedirect = true;
  $wnd.__gwt_activeModules['gui'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/gui?user.agent=' + ua + '&_callback=' + callback;
